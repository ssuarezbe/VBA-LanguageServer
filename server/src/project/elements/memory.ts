...
export abstract class DeclarationElement
  ...
  implements HasDiagnosticCapability
{
  ...
  static create(
    ...
    if (methodContext) {
      return new SubDeclarationElement(
        context,
    ...
    const propertyDeclaration = new PropertyDeclarationElement(
      context,
      document.textDocument,
    );
    const identifierText = propertyDeclaration.identifier.text;
    const predeclaredElements =
      document.currentScopeElement?.declaredNames.get(identifierText) ?? [];

    // Add to an existing property rather than creating.
    for (const element of predeclaredElements) {
      if (
        element.isPropertyElement() &&
        element.identifier.text === identifierText
      ) {
...
export class SubDeclarationElement
  extends DeclarationElement
  implements HasSymbolInformation
{
  ...
  constructor(
    context: ProcedureDeclarationContext,
    document: TextDocument,
    methodContext: SubroutineDeclarationContext,
  ) {
    super(context, document);

    const identifierContext = methodContext
      .subroutineName()
      ?.ambiguousIdentifier();
...
export class FunctionDeclarationElement
  extends DeclarationElement
  implements HasSymbolInformation
{
  identifier: IdentifierElement;
  ...
  constructor(
    context: ProcedureDeclarationContext,
    document: TextDocument,
    methodContext: FunctionDeclarationContext,
  ) {
    super(context, document);
    const identifierContext = methodContext
      .functionName()!
  ...
  implements HasSymbolInformation
{
  ...
  addPropertyDeclaration(
    ...
    switch (true) {
      case !!context.propertyGetDeclaration():
        propertyType = "Get";
        property = new PropertyGetDeclarationElement(
          context,
          document,
          context.propertyGetDeclaration()!,
        );
        this.getDeclarations.push(
          new PropertyGetDeclarationElement(
            context,
            document,
            context.propertyGetDeclaration()!,
          ),
      ...
      case !!context.propertySetDeclaration()?.LET():
        propertyType = "Let";
        property = new PropertyLetDeclarationElement(
          context,
          document,
          context.propertySetDeclaration()!,
        );
      ...
      default:
        ...
        this.setDeclarations.push(
          new PropertySetDeclarationElement(
            context,
            document,
            context.propertySetDeclaration()!,
          ),
  ...
  private _evaluateDuplicateDeclarationsDiagnostics(): void {
    [this.getDeclarations, this.letDeclarations, this.setDeclarations].forEach(
      (declarations) => {
        declarations.forEach((declaration, i) => {
          if (i > 0)
            this.diagnostics.push(
              new DuplicateDeclarationDiagnostic(declaration.identifier.range),
            );
        });
      },
    );
...
class PropertyGetDeclarationElement extends DeclarationElement {
  ...
  constructor(
    context: ProcedureDeclarationContext,
    document: TextDocument,
    getContext: PropertyGetDeclarationContext,
  ) {
    super(context, document);
    this.identifier = new IdentifierElement(
      getContext.functionName()!.ambiguousIdentifier()!,
      document,
    );
  }
}
...
class PropertyLetDeclarationElement extends DeclarationElement {
  ...
  constructor(
    context: ProcedureDeclarationContext,
    document: TextDocument,
    setContext: PropertySetDeclarationContext,
  ) {
    super(context, document);
    this.identifier = new IdentifierElement(
      setContext.subroutineName()!.ambiguousIdentifier()!,
      document,
    );
  }
}
...
abstract class BaseEnumDeclarationElement
  extends ScopeElement
  implements HasSemanticToken, HasSymbolInformation
{
  identifier: IdentifierElement;
  ...
  constructor(
    context: EnumDeclarationContext | EnumMemberContext,
    document: TextDocument,
  ) {
    super(context, document);
    this.identifier = new IdentifierElement(
      context.untypedName().ambiguousIdentifier()!,
      document,
    );
  }
}

export class EnumDeclarationElement
  extends BaseEnumDeclarationElement
  implements ScopeElement, HasDiagnosticCapability
{
  diagnostics: Diagnostic[] = [];
  ...
  get symbolInformation(): SymbolInformation {
    return SymbolInformationFactory.create(this, SymbolKind.Enum);
  }
  ...
  constructor(
    ...
    context
      .enumMemberList()
      .enumElement()
      .forEach((enumElementContext) =>
        this.pushDeclaredName(
          new EnumMemberDeclarationElement(
            enumElementContext.enumMember()!,
            document,
          ),
        ),
      );
  ...
  evaluateDiagnostics(): void {
    if (this.isDeclaredAfterMethod) {
      this.diagnostics.push(
        new ElementOutOfPlaceDiagnostic(this.range, "Enum declaration"),
      );
    }
  }
}
...
class EnumMemberDeclarationElement extends BaseEnumDeclarationElement {
  ...
  get symbolInformation(): SymbolInformation {
    return SymbolInformationFactory.create(this, SymbolKind.EnumMember);
  }
...
abstract class BaseVariableDeclarationStatementElement
  extends BaseContextSyntaxElement
  implements HasSemanticToken, HasSymbolInformation, NamedSyntaxElement
{
  tokenType: SemanticTokenTypes;
  tokenModifiers: SemanticTokenModifiers[] = [];
  ...
  constructor(
    context: VariableDclContext | ConstItemContext | UdtElementContext,
    document: TextDocument,
    tokenType: SemanticTokenTypes,
  ...
  implements HasSemanticToken, HasSymbolInformation, NamedSyntaxElement
{
  ...
  constructor(context: UdtDeclarationContext, document: TextDocument) {
    ...
    context
      ...
      .forEach((member) =>
        this.pushDeclaredName(
          new TypeMemberDeclarationElement(member, document),
        ),
      );
  }
...
export class TypeMemberDeclarationElement extends BaseVariableDeclarationStatementElement {
  ...
  constructor(context: UdtElementContext, document: TextDocument) {
    ...
    const identifierContext =
      context.udtMember()?.untypedNameMemberDcl()?.ambiguousIdentifier() ??
      context.udtMember()?.reservedNameMemberDcl()?.reservedMemberName();
    this.identifier = new IdentifierElement(identifierContext!, document);
  }
}
