...
export abstract class BaseProjectDocument {
  ...
  protected _documentScopeDeclarations: Map<string, Map<string, any>> =
    new Map();
  ...
  protected _semanticTokens: SemanticTokensManager =
    new SemanticTokensManager();
  ...
  get isOversize() {
    ...
    return (async () =>
      ...
      (await this.getDocumentConfiguration()).maxDocumentLines)();
  }
  ...
  async getDocumentConfiguration(): Promise<DocumentSettings> {
    ...
    if (this.workspace.hasConfigurationCapability) {
      this._documentConfiguration =
        await this.workspace.requestDocumentSettings(this.textDocument.uri);
      if (this._documentConfiguration) {
        return this._documentConfiguration;
      }
    }
  ...
  constructor(workspace: Workspace, name: string, document: TextDocument) {
    ...
    this.name = name;
    console.log("abstract class BaseProjectDocument -- Document builder done ");
  }
  ...
  static create(
    ...
    console.log(`processing file extension ${extension}`);
    switch (extension) {
      case "cls":
        return new VbaClassDocument(
          workspace,
          filename,
          document,
          SymbolKind.Class,
      ...
      default:
        throw new Error(
          "Expected *.cls, *.bas, or *.frm but got *." + extension,
        );
    }
  }
  ...
  languageServerDiagnostics(): PublishDiagnosticsParams {
    this._hasDiagnosticElements.forEach((e) => e.evaluateDiagnostics());
    return {
  ...
  async parseAsync(token: CancellationToken): Promise<void> {
    ...
    if (token.isCancellationRequested) {
      throw new ParseCancellationException(
        Error("Parse operation cancelled before it started."),
      );
    }
    ...
    token.onCancellationRequested(() => {
      throw new ParseCancellationException(
        new Error("Parse operation cancelled during parse."),
      );
    });
